...
abstract class CalDAVPlugin extends SyncPlugin
{
	...
	public function deleteEvent(string $eventId, string $calendarId): bool
	{
		...
		Factory::getApplication()->bootComponent('dpcalendar')->getMVCFactory()->createModel('Calendar', 'Administrator')->getCalendar($oldEvent->catid);
	...
	public function saveEvent(string $eventId, string $calendarId, array $data): string
	{
		...
		try {
			...
			if ($eventId !== '' && $eventId !== '0') {
				$event = $app->bootComponent('dpcalendar')->getMVCFactory()->createTable('Event', 'Administrator');
			...
			if (isset($data['location_ids'])) {
				// @phpstan-ignore-next-line
				$event->locations = $this->getDPCalendar()->getMVCFactory()->createModel('Geo', 'Administrator')->getLocations((array)$data['location_ids']);
			}
			$ical = $this->getDPCalendar()->getMVCFactory()->createModel('Ical', 'Administrator')->createIcalFromEvents([$event]);
			...
			if ($eventId === '' || $eventId === '0') {
				...
				if (isset($event->locations) && !empty($event->locations)) {
					$vevent->LOCATION = $this->getDPCalendar()->getMVCFactory()->createModel('Geo', 'Administrator')->format($event->locations);
