...
class PlgSearchDPCalendar extends JPlugin
{
	...
	public function onContentSearch($text, $phrase = '', $ordering = '', $areas = null)
	{
		...
		$sContent  = (bool) $this->params->get('search_content', 1);
		$sArchived = (bool) $this->params->get('search_archived', 1);
		$sPast	 = (bool) $this->params->get('pastevents', 1);
		...
		if ($limit > 0)
		{
			...
			if ($sPast === false)
			{
				$pastevents = ' AND a.start_date > ' . $db->quote($now);
			}
			...
			$query->select('a.id, a.title AS title, a.metadesc, a.metakey, a.created AS created, a.language, a.catid, a.description AS text')
				...
				->where(
					'(' . $where . ') AND ' . $state . ' AND c.published = 1 AND a.access IN (' . $groups . ') '
