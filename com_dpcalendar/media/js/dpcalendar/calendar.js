...
(function (document, window, Joomla, DPCalendar) {
  ...
  DPCalendar.createCalendar = function (calendar, options) {
    ...
    viewMapping['resday'] = 'resourceTimelineDay'; //BC < 7.2
    ...
    viewMappingReverse['resourceTimelineDay'] = 'resday'; //BC < 7.2

    viewMappingReverse['agendaWeek'] = 'week';
    viewMappingReverse['agendaDay'] = 'day'; // If there are resources use the proper timegrid views
    ...
    options['defaultDate'] = new Date(options['year'] + '-' + DPCalendar.pad(parseInt(options['month']), 2) + '-' + DPCalendar.pad(options['date'], 2));
    options['timeZone'] = Joomla.getOptions('DPCalendar.timezone');
    options['defaultView'] = viewMapping[options['defaultView']]; // Loading the list view when we have a small screen
    ...
    options['weekNumberTitle'] = ''; // Translations
    ...
    options['eventDrop'] = function (info) {
      ...
      DPCalendar.request('task=event.move&Itemid=' + Joomla.getOptions('DPCalendar.itemid'), function (json) {
        if (json.data.url) {
    ...
    options['dateClick'] = function (info) {
      ...
      if (form) {
        info.jsEvent.preventDefault(); // Setting some defaults on the quick add popup form
        ...
        end.actualDate = end.value; // Set location information
    ...
    cal.render(); // Toggle the list of calendars
