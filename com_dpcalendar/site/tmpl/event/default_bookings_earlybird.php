	...
	<?php foreach ($this->event->earlybird->value as $index => $value) { ?>
		...
		<div class="dp-earlybird dp-info-box">
			...
			<span class="dp-earlybird__content">
				<?php
				$value = ($this->event->earlybird->type[$index] == 'value' ? \DigitalPeak\Component\DPCalendar\Administrator\Helper\DPCalendarHelper::renderPrice($value) : $value . '%');
				$limit = $this->event->earlybird->date[$index];
				$date  = \DigitalPeak\Component\DPCalendar\Administrator\Helper\DPCalendarHelper::getDate($this->event->start_date);
				if (str_starts_with((string) $limit, '-') || str_starts_with((string) $limit, '+')) {
					// Relative date
					$date->modify(str_replace('+', '-', (string) $limit));
				} else {
					// Absolute date
					$date = \DigitalPeak\Component\DPCalendar\Administrator\Helper\DPCalendarHelper::getDate($limit);
