...
$format = \DigitalPeak\Component\DPCalendar\Administrator\Helper\DPCalendarHelper::getComponentParameter('event_date_format', 'd.m.Y') . ' '
	. \DigitalPeak\Component\DPCalendar\Administrator\Helper\DPCalendarHelper::getComponentParameter('event_time_format', 'H:i');
$return = '&return=' . base64_encode(Uri::getInstance()->toString());
?>
<div class="com-dpcalendar-taxrates__taxrates">
	<table class="dp-table" id="taxratesList">
		<thead>
		...
		<th><?php echo HTMLHelper::_('searchtools.sort', 'COM_DPCALENDAR_TAXRATE_FIELD_RATE_LABEL', 'a.rate', $listDirn, $listOrder); ?></th>
		...
		<?php foreach ($this->items as $i => $item) { ?>
			...
			<tr sortable-group-id="" data-draggable-group="0">
				...
				<td class="dp-table__col-expand" data-column="<?php echo $this->translate('JGLOBAL_TITLE'); ?>">
					...
					<?php } ?>
					<?php if ($canEdit) { ?>
						<a href="<?php echo Route::_('index.php?option=com_dpcalendar&task=taxrate.edit&r_id=' . (int)$item->id); ?>">
				...
				<td data-column="<?php echo $this->translate('COM_DPCALENDAR_CREATED_DATE'); ?>">
					<?php echo \DigitalPeak\Component\DPCalendar\Administrator\Helper\DPCalendarHelper::getDate($item->created)->format($format, true); ?>
				</td>
